...
/**
 * Force structure that stores force_creator_t function which actually
 * computes the force and adds it to a pair (or singular) of bodies,
 * aux_t structure that store auxiliary information necessary for running
 * the force_creator_t function, and a freer for aux.
 */
typedef struct force force_t;

/**
 * Initializes force structure with force creator, auxiliary data, and
 * auxiliary structure freer.
 *
 * @param creator force_creator_t function
 * @param aux aux_t structure
 * @param freer free_func_t aux freer
 * @return force_t*
 */
...
/**
 * @brief Frees memory stored by force structure pointer.
 *
 * @param force
 */
