...
void make_collision(void *aux) {
  ...
  if ((find_collision(shape1, shape2)).collided) {
    ...
    (aux_get_handler((aux_t *)aux))(body1, body2, axis,
                                    aux_get_aux((aux_t *)aux));
    // problem line -> maybe the handler?
  }
  list_free(shape1);
  list_free(shape2);
}
...
void create_destructive_collision(scene_t *scene, body_t *body1,
                                  body_t *body2) {
  create_collision(scene, body1, body1, make_destructive, NULL,
...
impulse_aux_t *impulse_aux_init(double elasticity, list_t *bodies,
                                vector_t axis) {
  impulse_aux_t *aux = malloc(sizeof(aux_t));
...
void create_collision(scene_t *scene, body_t *body1, body_t *body2,
  ...
  list_add(bodies, body2);
  aux_t *auxt = aux_init(0, bodies);
  aux_set_handler(auxt, handler);
  aux_set_aux(auxt, aux);
  scene_add_bodies_force_creator(scene, make_collision, auxt, bodies,
                                 (free_func_t)freer);
}

void create_destructive_collision(scene_t *scene, body_t *body1,
                                  body_t *body2) {
  create_collision(scene, body1, body2, NULL, NULL, NULL);
}
...
void make_phys_collision(body_t *body1, body_t *body2, vector_t axis,
  ...
  double comp_1 = vec_dot(vec_norm(axis), body_get_velocity(body1));
  double comp_2 = vec_dot(vec_norm(axis), body_get_velocity(body2));
  double impulse1_mag = body_get_mass(body1) * body_get_mass(body2) /
                        (body_get_mass(body1) + body_get_mass(body2)) *
                        (1 + ((aux_t *)aux)->c) * (comp_2 - comp_1);
  double impulse2_mag = body_get_mass(body1) * body_get_mass(body2) /
                        (body_get_mass(body1) + body_get_mass(body2)) *
                        (1 + ((aux_t *)aux)->c) * (comp_1 - comp_2);
  body_add_impulse(body1, vec_multiply(impulse1_mag, vec_norm(axis)));
  body_add_impulse(body2, vec_multiply(impulse2_mag, vec_norm(axis)));
}

// not sure where to check that a collision is actually occurring???
...
void create_physics_collision(scene_t *scene, double elasticity, body_t *body1,
  ...
  if (body_is_collided(body1) || body_is_collided(body2)) {
    ...
    collision_info_t *aux = malloc(sizeof(collision_info_t));
    *aux = collision;
    create_collision(scene, body1, body2, make_phys_collision, aux, NULL);
  }
